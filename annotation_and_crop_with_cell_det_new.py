...
train_img_dir = r"ELDDS1400c5-dataset-3/valid/images"
train_lbl_dir = r"ELDDS1400c5-dataset-3/valid/labels"

out_img_dir = r"cells_ELDDS1400c5-dataset-3/valid/images"
out_lbl_dir = r"cells_ELDDS1400c5-dataset-3/valid/labels"
...
def extract_cell_labels_from_pv(
    ...
    with open(pv_label_file, "r") as f:
        for line in f:
            ...
            # Calculate intersection between defect box and cell box
            ...
            # Use IoU (Intersection over Union) or intersection over defect area
            if defect_area > 0 and inter_area / defect_area >= min_overlap_ratio:
                # Clip the defect box to cell boundaries for cell-relative coordinates
                clip_x1 = inter_x1
                clip_y1 = inter_y1
                clip_x2 = inter_x2
                clip_y2 = inter_y2
                
                # Convert to cell-relative YOLO format (normalized 0-1)
...
for img_name in image_files:
    ...
    if num_wide > len(all_widths) * 0.2:  # >20% cells are too wide
        print("Too many wide cells → rerunning preprocessing with lower threshold", img_path)
    ...
    if len(boxes) < 20 :
        print('no detections or low so remove', img_path)
